...
from optuna_sampler_ocel import OptunaSamplers


DIRECTORY = './ICML/FINAL/FS_WITH_SKLEARN_ON_OCEl_WITH_HPO/'
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(columns=['LP', 'max_runtime', 'max_num_of_concepts_tested',
                               'iter_bound', 'quality_func', 'quality_score',
                               'Validation_f1_Score', 'Validation_accuracy'])
    ...
    for str_target_concept, examples in settings['problems'].items():
        ...
        st2 = time.time()
        optuna1 = OptunaSamplers(DATASET, kb_with_selected_features, lp, str_target_concept, val_pos, val_neg, df)
        ...
        wrap_obj = OcelWrapper(
            knowledge_base=kb_with_selected_features,
        ...
        # Log results
        with open(f'{DIRECTORY}{DATASET}_featureSelectionWithEvolearnerAndHPOCELOE.txt', 'a') as f:
            ...
            print('Time Taken for FS without one hot:', elapsed_time1, file=f)
            print('Time Taken for Fit and Predict:', elapsed_time3, file=f)
            print('Time Taken for HPO:', elapsed_time2, file=f)
            print('Best Hpo:', best_hpo, file=f)
            print('max runtime', best_hpo['max_runtime'].values[0])
            print('max_num_of_concepts_tested', best_hpo['max_num_of_concepts_tested'].values[0])
            print('iter_bound', best_hpo['iter_bound'].values[0])
            print('quality_func', best_hpo['quality_func'].values[0])
            print('Validation_f1_Score', best_hpo['Validation_f1_Score'].values[0])
            print('Validation_accuracy', best_hpo['Validation_accuracy'].values[0])
