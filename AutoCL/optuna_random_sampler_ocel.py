...
class OptunaSamplers():
    ...
    def write_to_df(self, **space):
        df.loc[len(df.index)] = [self.concept, space['max_runtime'],
                                 space['max_num_of_concepts_tested'], space['iter_bound'],space['quality_func'],
                                 space['quality_score'],
...
#1-25
    def objective(self, trial):
        ...
        max_num_of_concepts_tested = trial.suggest_int('max_num_of_concepts_tested', 2, 16)
        iter_bound = trial.suggest_int('iter_bound', 3, 26)
       

        # call the wrapper class
        wrap_obj = ocelWrapper(knowledge_base=kb,
    ...
    def objective_with_categorical_distribution(self, trial):
        ...
        max_runtime = trial.suggest_int("max_runtime", 1, 1000)
        max_num_of_concepts_tested = trial.suggest_int("max_num_of_concepts_tested", 2, 10_000)
        iter_bound = trial.suggest_int('iter_bound', 2, 10_000)
    
        quality_func = trial.suggest_categorical('quality_func', ['F1', 'Accuracy'])
        #heuristic_func = trial.suggest_categorical('heuristic_func',
                                                   #['CELOEHeuristic','OCELHeuristic',
                                                    #'DLFOILHeuristic'])
        # call the wrapper class
        wrap_obj = ocelWrapper(knowledge_base=kb,
        ...
        #space['heuristic_func'] = heuristic_func
    ...
    def objective_without_categorical_distribution(self, trial):
        ...
        max_num_of_concepts_tested = trial.suggest_int("max_num_of_concepts_tested", 2, 10)
        iter_bound = trial.suggest_int('iter_bound', 3, 25)
        
        quality_func = trial.suggest_int('quality_func', 1, 2)
       #heuristic_func = trial.suggest_int('heuristic_func', 1, 2,3)

        
        quality_func = 'F1' if quality_func >= 2 else 'Accuracy'
        #heuristic_func = 'CELOEHeuristic' if heuristic_func >= 2 else 'OCELHeuristic' if heuristic_func >= 3 else 'DLFOILHeuristic'
...
if __name__ == "__main__":
    for str_target_concept, examples in settings['problems'].items():
        ...
        wrap_obj = ocelWrapper(knowledge_base=kb,
                                     ...
                                     #heuristic_func=int(best_hpo['heuristic_func'].values[0])
                                   )
        print("************"+str(int(best_hpo['max_num_of_concepts_tested'].values[0])))
        model = wrap_obj.get_ocel_model()
